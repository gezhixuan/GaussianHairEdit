...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, step=1, max_cameras=None, load_depth=True):
    ...
    for idx, key in enumerate(subkeys):
        ...
        mask = None
        # mask_path = os.path.join(os.path.dirname(images_folder), 'alpha', image_name+'.png')
        # mask_img = Image.open(mask_path).convert("L")
        # mask = np.array(mask_img, dtype=np.float32) / 255.0
        
        # mask = None
        # if os.path.exists(mask_path):
        #     # load PNG and convert to float32 in [0, 1]
        #     mask_img = Image.open(mask_path).convert("L")
        #     mask = np.array(mask_img, dtype=np.float32) / 255.0
        # else:
        #     # if mask does NOT exist, use all-ones array
        #     mask = np.ones((height, width), dtype=np.float32)

        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image, depth=depth,
                              image_path=image_path, image_name=image_name, width=width, height=height, alpha=mask,)
