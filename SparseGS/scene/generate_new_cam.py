...
def create_cam_obj(cam, degree, R, T, scaling_factor,cam_focal_dict):
    ...
    if cam.alpha is not None:
        return Camera(colmap_id= None, R=R, T=T, FoVx = cam.FoVx, FoVy = cam.FoVy,  
            ...
            image_name=None, uid=uid, gt_alpha_mask = None, warp_mask = None, alpha = cam.alpha.clone().detach(),
                data_device = "cuda"
            )
    else:
        return Camera(colmap_id= None, R=R, T=T, FoVx = cam.FoVx, FoVy = cam.FoVy,  
            image=src_img, depth=scaled_depth, K = K, src_R = src_R, src_T = src_T, src_uid = cam.uid,
            image_name=None, uid=uid, gt_alpha_mask = None, warp_mask = None, alpha = None,
                data_device = "cuda"
            )
