...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, step, max_cameras, prune_sched):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        # --------------------------------------------------------
        # Mask everything with viewpoint_cam.alpha before any loss
        # --------------------------------------------------------
        alpha = viewpoint_cam.alpha  # expected [H, W] or [1, H, W], numpy or torch

        if alpha is not None:
            if not torch.is_tensor(alpha):
                alpha = torch.from_numpy(alpha)
            alpha = alpha.to(image.device, dtype=image.dtype)

            # Ensure shape [1, H, W] so it can broadcast to [C, H, W]
            if alpha.dim() == 2:
                alpha = alpha.unsqueeze(0)  # [1, H, W]

            # RGB-like images: [3, H, W] or [C, H, W]
            image_m       = image       * alpha
            gt_image_m    = gt_image    * alpha

            diff_image_m  = diff_image  * alpha if 'diff_image'  in locals() and diff_image  is not None else None
            warp_image_m  = warp_image  * alpha if 'warp_image'  in locals() and warp_image  is not None else None
            reg_gt_image_m= reg_gt_image* alpha if 'reg_gt_image' in locals() and reg_gt_image is not None else None

            # Depths: usually [1, H, W]. Use the single channel of alpha.
            depth_m = depth * alpha[:1] if depth is not None else None
            vp_depth_m = viewpoint_cam.depth * alpha[:1] if getattr(viewpoint_cam, "depth", None) is not None else None

            # Regularization mask, if you also want it clamped by alpha
            # reg_mask_m = reg_mask * alpha[:1] if 'reg_mask' in locals() and reg_mask is not None else reg_mask
            reg_mask_m = reg_mask * alpha[:1] if 'reg_mask' in locals() and reg_mask is not None else None

            # print(reg_mask)
            # print(reg_mask_m)
            
        else:
            # Fallback: no alpha, just use originals
            image_m        = image
            gt_image_m     = gt_image
            diff_image_m   = diff_image  if 'diff_image'  in locals() else None
            warp_image_m   = warp_image  if 'warp_image'  in locals() else None
            reg_gt_image_m = reg_gt_image if 'reg_gt_image' in locals() else None
            depth_m        = depth
            vp_depth_m     = getattr(viewpoint_cam, "depth", None)
            reg_mask_m     = reg_mask if 'reg_mask' in locals() else None
        ...
        if dataset.lambda_pearson > 0 and (not pick_warp_cam) and depth_m is not None and vp_depth_m is not None:
            pearson_loss = pearson_depth_loss(depth_m.squeeze(0), vp_depth_m.squeeze(0))
            loss += dataset.lambda_pearson * pearson_loss
        
        if dataset.lambda_local_pearson > 0 and (not pick_warp_cam) and depth_m is not None and vp_depth_m is not None:
            lp_loss = local_pearson_loss(depth_m.squeeze(0), vp_depth_m.squeeze(0),
                                         dataset.box_p, dataset.p_corr)
            loss += dataset.lambda_local_pearson * lp_loss
        
        if pick_diff_cam and diff_image_m is not None:
            diffusion_loss = guidance_sd.train_step(diff_image_m.unsqueeze(0), dataset.step_ratio)
            loss += dataset.lambda_diffusion * diffusion_loss

        if pick_warp_cam and warp_image_m is not None and reg_gt_image_m is not None:
            # reg_mask_m is already alpha-masked above
            reg_Ll1 = mask_l1_loss(warp_image_m, reg_gt_image_m, reg_mask_m)
            reg_loss = (1.0 - opt.lambda_dssim) * reg_Ll1 + opt.lambda_dssim * (1.0 - ssim(warp_image_m, reg_gt_image_m))
            loss += dataset.lambda_reg * reg_loss
