...
def l1_loss_nonzero(network_output, gt):
    # mask where gt > 0
    mask = gt > 0

    # avoid empty mask causing .mean() NaN
    if not mask.any():
        return torch.tensor(0.0, device=network_output.device, dtype=network_output.dtype)

    # apply mask and compute L1 only on masked elements
