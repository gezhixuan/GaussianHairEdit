...
# cd "$PROJECT_DIR"
# conda deactivate && conda activate zx_3d
# python src/filter_scene_with_clip.py clip_filter \
#     --src_scene "$REGISTER_SCENE_DIR" \
#     --dst_scene "$CLIP_SCENE_DIR" \
#     --text_prompt "$CLIP_PROMPT" \
...
cd "$PROJECT_DIR/SparseGS"
conda deactivate && conda activate instantsplat
CUDA_VISIBLE_DEVICES="$GPU" python train.py --source_path "$CLIP_SCENE_DIR"  --model_path "$CLIP_SCENE_DIR/SparseGS" \
