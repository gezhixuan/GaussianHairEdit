...
class GaussianEditor(BaseLift3DSystem):
    ...
    @torch.no_grad()
    # use SAM to get the mask for delete and editing
    ...
    def forward_spec(self, gaussian, batch: Dict[str, Any], renderbackground=None, local=False,
                     with_semantic=True, override_color=None) -> Dict[str, Any]:
        if renderbackground is None:
            renderbackground = self.background_tensor
        images = []
        depths = []
        semantics = []
        masks = []
        gaussian.localize = local

        for id, cam in enumerate(batch["camera"]):
            render_pkg = render(cam, gaussian, self.pipe, renderbackground, override_color=override_color)
            image, viewspace_point_tensor, _, radii = (
                render_pkg["render"],
                render_pkg["viewspace_points"],
                render_pkg["visibility_filter"],
            ...
            if with_semantic:
                semantic_map = render(
                    ...
                    self.pipe,
                    renderbackground,
                    override_color=gaussian.mask[..., None].float().repeat(1, 3),
                )["render"]
                semantic_map = torch.norm(semantic_map, dim=0)
                semantic_map = semantic_map > 0.3
                semantic_map_viz = image.detach().clone()
        ...
        gaussian.localize = False  # reverse
    ...
    def forward_noise(self, gaussian, batch: Dict[str, Any], background):
        (H, W, C) = background.shape
        point_xyz = gaussian.get_xyz
        point_color = torch.randn((point_xyz.shape[0], C), device=point_xyz.device)
        assert len(batch['camera']) == 1

        for id, cam in enumerate(batch["camera"]):
            transform_matrix = cam.full_proj_transform
            hom_xyz = torch.matmul(torch.cat([point_xyz, torch.ones_like(point_xyz[:, :1])], dim=-1), transform_matrix)
            hom_xyz = hom_xyz[:, :3] / (hom_xyz[:, 3:] + 1e-5)
            pixel_x = ((hom_xyz[:, 0] + 1.0) * W - 1.0) * 0.5
            pixel_y = ((hom_xyz[:, 1] + 1.0) * H - 1.0) * 0.5
            pixel_z = hom_xyz[:, 2]
            visible_mask = (pixel_x >= - 0.5) & (pixel_x <= W - 0.5) \
                           & (pixel_y >= - 0.5) & (pixel_y <= H - 0.5) \
                           & (pixel_z >= 0.2)

            pixel_x = torch.round(pixel_x).to(torch.int64)
            pixel_y = torch.round(pixel_y).to(torch.int64)

            visible_pixel_x = pixel_x[visible_mask]
            visible_pixel_y = pixel_y[visible_mask]
            visible_point_color = point_color[visible_mask]
            visible_pixel_z = pixel_z[visible_mask]

            image = torch.zeros((H, W, C), device=point_color.device)  # 初始化一个 HxW 的黑色图像

            sorted_indices = torch.argsort(visible_pixel_z)
            visible_pixel_x = visible_pixel_x[sorted_indices]
            visible_pixel_y = visible_pixel_y[sorted_indices]
            visible_point_color = visible_point_color[sorted_indices]
            visible_pixel_z = visible_pixel_z[sorted_indices]

            for x in range(W):
                for y in range(H):
                    select_mask = (visible_pixel_x == x) & (visible_pixel_y == y)
                    if select_mask.sum() > 0:
                        image[y, x] = visible_point_color[select_mask][torch.argmin(visible_pixel_z[select_mask])]
                    else:
                        image[y, x] = background[y, x]
        return image.permute(2, 0, 1).unsqueeze(0)

    def render_all_view(self, cache_name, gaussian=None, with_semantic=True, override_color=None):
        render_frames = {}
            ...
            for id in tqdm(self.view_list):
                ...
                if not os.path.exists(cur_path) or self.cfg.cache_overwrite:
                    ...
                    if gaussian is None:
                        ...
                        out = self.forward_spec(gaussian, cur_batch,
                                                with_semantic=with_semantic,
                                                override_color=override_color)["comp_rgb"]
                    out_to_save = (
                ...
                render_frames[id] = torch.tensor(
                    cached_image / 255, device="cuda", dtype=torch.float32
                )[None]
        return render_frames

    def render_all_view_no_cache(self, gaussian=None, with_semantic=True, override_color=None):
        render_frames = {}
        with torch.no_grad():
            for id in tqdm(self.view_list):
                cur_cam = self.trainer.datamodule.train_dataset.scene.cameras[id]
                ...
                if gaussian is None:
                    raise ValueError("gaussian should not be None, if so the viewspace_point_tensor will be overrided")
                else:
                    out = self.forward_spec(gaussian, cur_batch,
                                            with_semantic=with_semantic,
                                            override_color=override_color)["comp_rgb"]
                render_frames[id] = out
        return render_frames

    def render_all_noise(self, gaussian):
        background = torch.randn((64, 64, 4), device="cuda")
        render_frames = {}
        with torch.no_grad():
            for id in tqdm(self.view_list):
                cur_cam = self.trainer.datamodule.train_dataset.scene.cameras[id]
                ...
                render_frames[id] = self.forward_noise(gaussian, cur_batch, background)
        return render_frames
